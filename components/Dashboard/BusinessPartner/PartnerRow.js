...
const PartnerRow = ({ data, ind }) => {
  return (
    <div
      className={`grid grid-cols-7 h-16 text-gray-700 items-center font-medium ${
        ind % 2 != 0 ? 'bg-gray-100' : ''
      }`}
    >
      <div className='pl-2 text-start col-span-1'>{data?.id}</div>
      <div className='pl-2 text-start col-span-1'>{data?.name}</div>
      <div className='pl-2 text-start col-span-1'>{data?.email}</div>
      <div className='pl-2 text-start col-span-1'>{data?.phone_number}</div>
      <div className='pl-2 text-start col-span-1'>
